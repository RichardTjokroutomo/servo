...
pub(crate) struct TextFragment {
    ...
    /// 1. The width of the containing block. We need this later to check where to clip the text during 
    /// display list construction.
    pub parent_width: Au,

    // 2. The width of left & right text overflow marker. CSS specs refers to them as `first` & `second`.
    // When `text-overflow: ellipsis`, right clip is the width of the ellipsis glyph.
    pub overflow_marker_width: (Au, Au),

    /// 3. Per the CSS specification <https://drafts.csswg.org/css-overflow/#text-overflow>,
    ...
    /// character, then we clip instead of ellide it.
    pub contains_first_character_of_the_line: bool,

    /// 4. A line can contain more than one TextFragment. 
    ...
    /// 5. Can this TextFragment be ellided? 
