...
impl LineItemLayout<'_, '_> {
    ...
    fn layout_text_run(&mut self, text_item: TextRunLineItem) {
        ...
        for (glyph_index, glyph_store) in text_item.text.iter().enumerate() {
            ...
            if incoming_tab_index < text_item.tab_stop_indices.len() && tab_index_needs_updating {
                inline_advance += self.distance_to_next_tab_stop(glyph_space_advance * 8); // TODO: POC. fix this later. 
            ...
            if glyph_index + 1 == incoming_tab_stop {
    ...
    fn distance_to_next_tab_stop(&self, tab_width: Au) -> Au {
        return tab_width - (self.current_state.inline_advance % tab_width);
    }
