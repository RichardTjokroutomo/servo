...
use crate::fragment_tree::{
    BaseFragmentInfo, BoxFragment, Fragment, OverflowIndicatorData, TextFragment,
};
...
impl LineItemLayout<'_, '_> {
    ...
    fn layout_text_run(
        ...
        if can_be_elided {
            ...
            let mut overflow_indicator_font = None;
            let mut overflow_font_instance_key = None;
            for c in overflow_indicator_string.chars() {
                let Some((overflow_indicator_textrun_segment, font, font_instance_key)) =
                    self.form_overflow_indicator(&c.to_string())
                else {
                ...
                overflow_indicator_font = Some(font);
    ...
    fn form_overflow_indicator(
        ...
        let mut overflow_indicator_font_cache: Vec<FontKeyAndMetrics> = vec![];
        ...
        overflow_indicator_font_cache.push(FontKeyAndMetrics {
            metrics: overflow_indicator_font.metrics.clone(),
            key: overflow_font_instance_key,
            pt_size: overflow_indicator_font.descriptor.pt_size,
        });
        let overflow_indicator_font_index = overflow_indicator_font_cache.len() - 1;

        // 2. Create the `TextRunSegment`
        let mut overflow_indicator_textrun_segment = TextRunSegment::new(
            overflow_indicator_font_index,
