...
impl PlatformFontMethods for PlatformFont {
    ...
    fn glyph_h_advance(&self, glyph: GlyphId) -> Option<FractionalPixel> {
        ...
        if unsafe { FT_IS_SCALABLE(face.as_ptr()) } {
            let advance = unsafe { (*slot).linearHoriAdvance };
            return Some(fixed_16_dot_16_to_float(advance) * self.unscalable_font_metrics_scale());
        } else {
            let advance = unsafe { (*slot).metrics.horiAdvance };
            return Some(fixed_26_dot_6_to_float(advance) * self.unscalable_font_metrics_scale());
        }
    }
